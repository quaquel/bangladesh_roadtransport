...
class FederateStarter():
        ...
        while loop:
            ...
            if error:
                ...
                if message[4][1] == "FM.1": #FEDERATE START MESSAGE
                    ...
                    logger.info("Federate start message has been received for model {}".format(model_id))
                    # === request status ===
                    wait = True
                    while wait:
    ...
    def start_federate(self, payload):
        ...
        in_use = True
        while in_use:
            m_port = random.randint(self.min_port, self.max_port) #the range of unregistered ports: 49152, 65535, also the range zmq.bind_random uses
        ...
        identity = u"%04x-%04x" % (random.randint(0, 0x10000), random.randint(0, 0x10000))#identity as a string with double integers
    ...
    def RequestStatus(self, sim_run_id, receiver_id):
        ...
        try:
            ...
            #logger.info("Model status received by the FS: ", r_message)
            expected_type = "MC.1"
