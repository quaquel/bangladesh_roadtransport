...
from ema_workbench.em_framework import (Scenario, Policy, RealParameter, IntegerParameter, Constant, perform_experiments)

from ema_workbench.util.utilities import save_results, load_results
from ema_workbench.util import CaseError, EMAError, ema_logging
from ema_workbench.util.ema_logging import debug, info, warning

import ema_workbench.em_framework.samplers as samplers
import ema_workbench.em_framework.util as util

from zmq.error import ZMQError
#from examples.lake_model import lake_problem
from connector.Server import Server
from connector.message_v2 import message_encode, message_decode
...
class ZeroMQModel(AbstractModel):
    ...
    def __init__(self, name, softwareCode,
        ...
        name : name of the model, e.g. "WB_transport"
        wd : working directory to the .jar file (wherever the model is)
        ...
        self.no_messages = 0 #everytime this ZeroMQModel object sends a message, this variable will be incremented
    ...
    def model_init(self, policy):
        ...
        self.fs_socket = self.context.socket(zmq.REQ)
        #print("in model_init, setting the identity")
        identity = u"%04x-%04x" % (randint(0, 0x10000), randint(0, 0x10000))
        self.fs_socket.setsockopt_string(zmq.IDENTITY, identity)
        try:
        ...
        self.m_socket = self.context.socket(zmq.REQ)
        identity = u"%04x-%04x" % (randint(0, 0x10000), randint(0, 0x10000))
        self.m_socket.setsockopt_string(zmq.IDENTITY, identity)
        ...
        payload = [self.m_receiver, self.softwareCode, self.argsBefore, self.model_file, self.argsAfter,
                   self.fs_workingDirectory, self.redirectStdin, self.redirectStdout, self.redirectStderr, 
                   self.deleteWorkingDirectory, self.deleteStdout, self.deleteStdin]
        
        self.FederateStart(payload)
        self.m_socket.connect("tcp://{}:{}".format(self.ip_toconnect, self.m_port))
    ...
    def run_model(self, scenario, policy, experiment_id):
        ...
        experiment_id : the number of simulation we will be running. Message structure requires this. 
                        Starts with 0!
                        IT REQUIRES MODIFYING THE EXPERIMENT_RUNNER.run_experiment()
        '''
        super(ZeroMQModel, self).run_model(scenario, policy)
