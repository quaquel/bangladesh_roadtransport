...
class FederateStarter():
    ...
    def __init__(self, federation_name, magic_no, fm_port, sender_id, min_port, max_port):
        '''
        ...
        self.model_processes = {} # dictionary in the {model_id : (process, socket, port_no)} format 
                                  #model ids and their corresponding processes are kept in a dictionary, so that they can be accessed easily when necessary, e.g. for termination
        ...
        while loop:
            ...
            address, empty, b_message = self.fm_socket.recv_multipart()
            #b_message = self.fm_socket.recv()
            message = message_decode(b_message)        
#             print("a message has been received: ", message)
            ...
            if error:
                ...
                if message[4][1] == "FM.1": #FEDERATE START MESSAGE
                    ...
                    self.FederateStarted(sim_run_id, fm_id, address, model_id, status, m_port, error_msg)
    ...
    def start_federate(self, payload):
        #payload = [instanceId, softwareCode, argsBefore, self.model_file, argsAfter,
        #           workingDirectory, m_port, redirectStdin, redirectStdout, redirectStderr, 
        #           deleteWorkingDirectory, deleteStdout, deleteStdin]
        ...
        with open(redirectStdout, 'w') as f1, open(redirectStderr, 'w') as f2:
            try:
                process = subprocess.Popen([softwareCode, argsBefore, model_file, str(instanceId), str(m_port)],
                                       stdout=f1, stderr=f2)
    ...
    def FederateStarted(self, sim_run_id, receiver_id, address, model_id, status, m_port, error_msg):
        content = self.prepare_message(sim_run_id=sim_run_id, 
                                       ...
                                       type='FS.2',
                                       status=1,
                                       payload=[model_id, status, error_msg, m_port])
